        ...
        setTimeout(() => {
            ...
            if (media) {
                ...
                if (media.tagName === 'IMG') {
                    ...
                    media.querySelector('source').src = "flowers(MP4).mp4";
                    media.load();
                }
            }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!localStorage.getItem('mediaViewed')) {
        document.querySelector('.media-item').classList.add('first-time');
    ...
    function createExplosion(x, y, color) {
        ...
        for (let i = 0; i < particleCount; i++) {
            ...
            particle.style.width = `${Math.random() * 5 + 2}px`; // Smaller particles (2-7px)
            particle.style.height = particle.style.width;
            particle.style.background = color || `hsl(${Math.random() * 360}, 100%, 70%)`;
            particle.style.left = `${x}px`;
            particle.style.top = `${y}px`;
            document.body.appendChild(particle);
            
            const angle = Math.random() * Math.PI * 2;
            const velocity = Math.random() * 8 + 3; // Slightly faster particles
    ...
    // Open media viewer with explosion animation
    mediaItems.forEach(item => {
        item.addEventListener('click', function(e) {
            // Mark that user has interacted
            localStorage.setItem('mediaViewed', 'true');
            item.classList.remove('first-time');
            ...
            if (img) {
                ...
                createExplosion(e.clientX, e.clientY, '#ffffff');
                
                // Hide the clicked image with explosion animation
                item.style.animation = 'explode 0.4s forwards';
                
                // Set the fullscreen image source
                fullscreenImage.src = img.src;
                fullscreenImage.alt = img.alt;
                
                // Show the viewer with slide-up animation
                setTimeout(() => {
    ...
    const lazyLoadImages = () => {
        const lazyImages = document.querySelectorAll('.media-item img');
